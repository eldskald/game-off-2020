...
uniform float wavelength: hint_range(0.001, 20.0) = 1.0;
uniform float frequency: hint_range(0.0, 20.0) = 0.0;
uniform float phase: hint_range(0.0, 10.0) = 0.0;

void fragment() {
	vec2 uv = floor(UV/TEXTURE_PIXEL_SIZE);
	float dx = amplitude*sin(PI*(uv.y + (TIME + phase)*frequency)/wavelength);
